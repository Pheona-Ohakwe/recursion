...
def calculate_total_folder_size(folder):
    ...
    for item in folder:
        if isinstance(item, int):
            total_size += item  # If item is a file (integer), add its size
        elif isinstance(item, list):
            total_size += calculate_total_folder_size(item)  # Recursively calculate size of subfolder
    return total_size

# Example usage with provided file system
file_system = [
    [10, 20, 30],       # Folder 1 with files of sizes 10, 20, and 30
    [15, [25, 35]],     # Folder 2 with a file of size 15, and another folder with files of size 25, and 35
    40,                 # File 1 with size 40
...
print(calculate_total_folder_size(file_system))  # Output: 410 (sum of all file sizes)
